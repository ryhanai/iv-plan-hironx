...
myserviceprovider_spec = ["implementation_id", "HandScanRecognition",
                          "type_name",         "HandScanRecognition",
...
class MyServiceSVC_impl(_GlobalIDL__POA.RecognitionService):
  ...
  def setModelID(self, modelID):
    ...
    try:
      ...
      self._frames = toFloatArray(fs[0])
    except:
      print 'recognition failed'

class HandScanRecognition(OpenRTM_aist.DataFlowComponentBase):
  ...
  def onInitialize(self):
    ...
    self._myServicePort.registerProvider("recogPort", "RecognitionService", self._myservice0)
...
def HandScanRecognitionInit(manager):
...
def MyModuleInit(manager):
  HandScanRecognitionInit(manager)

  # Create a component
  comp = manager.createComponent("HandScanRecognition")
